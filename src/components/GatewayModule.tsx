...
import { SystemSettings, UserRole, Product } from "../types";
...
export default function GatewayModule({
  ...
  const handleSendWhatsAppMessage = async (targetPhone: string, text: string, type: "test" | "low_stock") => {
    if (!targetPhone) {
          ...
          <div className="p-5 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {whatsappProvider !== "DIRECT_LINK" && (
                <div className="space-y-1.5 animate-in fade-in duration-200">
                  <label className="text-xs font-bold text-slate-500 uppercase">URL do Endpoint da API</label>
                  <input
                    type="url"
                    disabled={!canEdit || !whatsappEnabled}
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 font-mono text-xs outline-none text-slate-800 disabled:opacity-60"
                    placeholder={whatsappProvider === "EVOLUTION_API" ? "https://api.seuservidor.com/message/sendText/instancia" : "https://api.twilio.com/2010-04-01/Accounts/..."}
                  />
                  <p className="text-[10px] text-slate-400">Endpoint HTTP POST para processamento do envio da mensagem.</p>
                </div>
              )}
            </div>
            ...
            {whatsappProvider !== "DIRECT_LINK" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-100 pt-4 mt-2 animate-in fade-in duration-200">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 uppercase">Token de Autorização (API Key / secret)</label>
                  <input
                    type="password"
                    disabled={!canEdit || !whatsappEnabled}
                    value={whatsappToken}
                    ...
                    placeholder="••••••••••••••••••••••••"
                  />
                  <p className="text-[10px] text-slate-400">Chave Bearer ou API Secret para autenticação no gateway.</p>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 uppercase">ID do Telefone ou Nome da Instância</label>
                  <input
                    type="text"
                    disabled={!canEdit || !whatsappEnabled}
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 font-mono text-xs outline-none text-slate-800 disabled:opacity-60"
                    placeholder={whatsappProvider === "EVOLUTION_API" ? "MinhaInstancia" : "Ex: MG1a2b3c4d..."}
                  />
                  <p className="text-[10px] text-slate-400">Identificador exclusivo do canal de envio de mensagens.</p>
                </div>
              </div>
            )}
          </div>
